...
func NewSecure(root hasharry.Hash, db Database, cachelimit uint16) (*SecureTrie, error) {
	if db == nil {
...
func (t *SecureTrie) TryUpdate(key, value []byte) error {
	...
	t.getSecKeyCache()[string(hk)] = hasharry.CopyBytes(key)
	return nil
}
...
// from the database.
func (t *SecureTrie) Commit() (root hasharry.Hash, err error) {
	return t.CommitTo(t.trie.db)
}

func (t *SecureTrie) Hash() hasharry.Hash {
	return t.trie.Hash()
}
...
func (t *SecureTrie) CommitTo(db DatabaseWriter) (root hasharry.Hash, err error) {
