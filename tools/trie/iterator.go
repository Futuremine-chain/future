...
type nodeIteratorState struct {
	hash    hasharry.Hash // Hash of the node being iterated (nil if not standalone)
	node    node          // Trie node being iterated
	parent  hasharry.Hash // Hash of the first full ancestor node (nil if current is the root)
...
func (it *nodeIterator) Hash() hasharry.Hash {
	if len(it.stack) == 0 {
		return hasharry.Hash{}
	}
	return it.stack[len(it.stack)-1].hash
}

func (it *nodeIterator) Parent() hasharry.Hash {
	if len(it.stack) == 0 {
		return hasharry.Hash{}
	}
...
func (st *nodeIteratorState) resolve(tr *Trie, path []byte) error {
	if hash, ok := st.node.(hashNode); ok {
		...
		st.node = resolved
		st.hash = hasharry.BytesToHash(hash)
	}
	return nil
}

func (it *nodeIterator) nextChild(parent *nodeIteratorState, ancestor hasharry.Hash) (*nodeIteratorState, []byte, bool) {
		...
		for i := parent.index + 1; i < len(node.Children); i++ {
			...
			if child != nil {
				...
				state := &nodeIteratorState{
					hash:    hasharry.BytesToHash(hash),
					node:    child,
					parent:  ancestor,
					index:   -1,
					pathlen: len(it.path),
...
func (it *differenceIterator) Hash() hasharry.Hash {
	return it.b.Hash()
}

func (it *differenceIterator) Parent() hasharry.Hash {
	return it.b.Parent()
}
...
func (it *differenceIterator) Next(bool) bool {
	...
	for {
		...
		case 0:
			...
			hasHash := it.a.Hash() == hasharry.Hash{}
...
func (it *unionIterator) Hash() hasharry.Hash {
	return (*it.items)[0].Hash()
}

func (it *unionIterator) Parent() hasharry.Hash {
...
func (it *unionIterator) Next(descend bool) bool {
	...
	for len(*it.items) > 0 && ((!descend && bytes.HasPrefix((*it.items)[0].Path(), least.Path())) || compareNodes(least, (*it.items)[0]) == 0) {
		...
		if skipped.Next(skipped.Hash() == hasharry.Hash{}) {
			it.count += 1
...
func (it *prefixIterator) Hash() hasharry.Hash {
	if it.hasPrefix() {
...
func (it *prefixIterator) Parent() hasharry.Hash {
	if it.hasPrefix() {
